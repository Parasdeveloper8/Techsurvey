...
var Storeofavdev *sessions.CookieStore
...
func init() {
	...
	Storeofavdev = sessions.NewCookieStore(secretKey)
	Storeofavdev.Options = &sessions.Options{
		Path:     "/",
...
func HandleFavDevSurveySubmission(c *gin.Context) {
	// Retrieve the session from the context or directly from the store
	session, err := Storeofavdev.Get(c.Request, "login-session") // Use Store here
	if err != nil {
	...
	favdev := c.PostForm("favp")
	currentDate := time.Now()
	// Insert data into the database
	query := "INSERT INTO techsurvey.favdev (email, vote, date, number) VALUES (?, ?, ?, ?)"
	result, err := db.Exec(query, sessionEmail, favdev, currentDate, number) // Include number in query
	if err != nil {
	...
	body := fmt.Sprintf("Hi,\n\nThank you for completing the survey!\nYour favorite development type: %s\nYour response: %s\n\nWe appreciate your time.", favdev, number)
